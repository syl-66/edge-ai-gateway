...
/* ============================================================
 * handle: control_relay — 控制继电器风扇 (开关 + PWM 调速)
 *   args 中解析:
 *     "action": "on"/"off"   — 继电器开关
 *     "speed":  0~100        — 风扇转速百分比 (PWM)
 * ============================================================ */

static int _parse_int_arg(const char *args, const char *key, int default_val) {
...
int tool_control_relay(const char *args, char *result, int len) {
    int on = 0;
    if (strstr(args, "\"on\"") || strstr(args, "on")) on = 1;
    if (strstr(args, "\"off\"")) on = 0;

    /* 控制继电器 (电源通断) */
    device_relay_set(RELAY_FAN, on);

    if (on) {
        /* 解析转速百分比, 默认全速 */
        int speed = _parse_int_arg(args, "speed", 100);
        fan_pwm_set_speed(speed);
        snprintf(result, len, "{\"success\":true,\"state\":\"on\",\"speed\":%d}", speed);
    } else {
        fan_pwm_set_speed(0);
