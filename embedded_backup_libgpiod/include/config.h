 ...
 * │ 模块       │ 访问方式     │ 原因                         │
 * ├────────────┼──────────────┼──────────────────────────────┤
 * │ DHT11      │ /dev/mem mmap│ 单总线 bit-banging, 需 μs 级 │
 * │ IR 发射    │ /dev/mem mmap│ 38kHz 载波, 需 8μs 精确定时  │
 * │ 继电器     │ libgpiod     │ 低频开关, 标准 GPIO 操作      │
 * │ 状态 LED   │ libgpiod     │ 低频闪烁, 标准 GPIO 操作      │
 * │ 按键       │ libgpiod     │ 边沿中断, libgpiod 事件机制   │
 * └────────────┴──────────────┴──────────────────────────────┘
 *
 * libgpiod: /dev/gpiochipN 字符设备, 适用于常规 GPIO
 * mmap:     /dev/mem 直接寄存器映射, 适用于高速 bit-banging
 */

/* DHT11 温湿度 (GPIO1_IO03, /dev/mem 直接寄存器操作) */
#define DHT11_GPIO          3

/* 继电器 (GPIO1_IO05, libgpiod 控制, 经三极管驱动风扇) */
#define RELAY_FAN_GPIO      5

/* IR 发射 (GPIO1_IO18, /dev/mem 直接寄存器操作, NEC 编码 + 38kHz 载波) */
#define IR_TX_GPIO          18

/* 状态 LED (GPIO1_IO02, libgpiod 控制, 系统运行状态指示) */
#define STATUS_LED_GPIO     2

/* 按键 (GPIO1_IO11/IO12, libgpiod 输入 + 边沿中断) */
#define KEY1_GPIO           11
#define KEY2_GPIO           12
