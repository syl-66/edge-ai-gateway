 ...
 *   - w25q32.c  (SPI)
 *   - status_led.c (GPIO libgpiod 状态灯)
 */
...
int sensor_manager_init(void) {
    ...
    if (dht11_init(DHT11_GPIO) < 0) {
        ...
        LOG_INFO(" [GPIO] DHT11 就绪 (GPIO%d, /dev/mem mmap)", DHT11_GPIO);
    }
    ...
    if (g_i2c_fd < 0) {
        LOG_ERROR(" [I2C] BH1750 初始化失败 (总线=%s 地址=0x%02X)",
                I2C_BUS, BH1750_ADDR);
        /* 建议排查: i2cdetect -y 1 确认设备存在; 检查上拉电阻 */
    } else {
...
int sensor_read_all(sensor_value_t *out, int max_count) {
    ...
    static int dht11_warn_once = 0;  /* 只 warn 一次 */

    /* ---- 读 DHT11 (GPIO, /dev/mem mmap) ---- */
    if (!dht11_disabled) {
        ...
        } else if (!dht11_warn_once) {
            dht11_warn_once = 1;
            LOG_WARN("DHT11 读取失败 (检查接线+上拉电阻, /dev/mem 模式, GPIO1_IO%d)", DHT11_GPIO);
        }
    }
    ...
    if (g_i2c_fd >= 0) {
        ...
        if (lux >= 0) {
            ...
            /* 第一次读失败就关闭, 后续跳过 (芯片未接/通信异常) */
            LOG_WARN("BH1750 读取持续失败, 已禁用");
            close(g_i2c_fd);
            g_i2c_fd = -1;
        }
    }
...
void sensor_manager_cleanup(void) {
    /* DHT11 (GPIO, /dev/mem + mmap): 调用 dht11_cleanup 释放 mmap 和 memfd */
    dht11_cleanup();
