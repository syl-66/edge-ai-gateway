...
int main(void)
{
    ...
    sqlite_storage_init(DB_PATH);
    // MQTT客户端初始化：连接MQTT消息服务器，绑定当前设备唯一ID
    mqtt_client_init(MQTT_BROKER_HOST, MQTT_BROKER_PORT, DEVICE_ID);
    // 指令分发工具模块初始化：用于解析云端下发控制指令、执行对应设备操作
    ...
    while (g_running)
    {
        ...
        // 填充当前设备唯一ID
        snprintf(report.id, sizeof(report.id), "%s", DEVICE_ID);
        ...
        // 向MQTT服务端发布JSON数据
        mqtt_client_publish(topic, json_buf);

        /**************************** SQLite本地持久化存储 **********************/
        // 遍历本轮所有传感器数据，逐条插入数据库保存历史记录
        for (int i = 0; i < sensor_count; i++)
        {
            sqlite_storage_insert_sensor(sensor_vals[i].name,
                                         sensor_vals[i].value,
                                         sensor_vals[i].unit);
        }

        // MQTT非阻塞事件循环100ms，处理心跳保活、断线重连、订阅下行消息
        // 防止主线程sleep阻塞导致MQTT连接断开
        ...
        {
            sqlite_storage_cleanup_old_data(DB_RETENTION_DAYS);
            last_cleanup = now;
        }
    ...
    mqtt_client_cleanup();       // 断开MQTT连接销毁客户端
    sqlite_storage_close();      // 正常关闭数据库文件
    tool_dispatcher_cleanup();   // 释放指令分发模块内存
