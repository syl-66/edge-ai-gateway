...
> 你对网关说"晚上温度降到 18°C 以下就开暖气，白天湿度低于 30% 就开加湿器"——Agent 自动理解意图，创建规则，持续监控传感器，自主执行。
...
- [ ] 边缘端本地推理（ONNX Runtime 跑小模型，离线也能用）
- [ ] 语音交互（ASR + TTS，真正的"对话式"智能家居）
- [ ] 多设备协同（多个 i.MX6ULL 节点，Agent 统一调度）
- [ ] 规则引擎可视化（前端拖拽创建自动化规则）
